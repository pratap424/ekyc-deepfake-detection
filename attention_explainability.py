...
class EnhancedFaceVerifier:
    ...
    def verify_with_explanation(self, image1, image2):
        ...
        """
        # Get verification result from base verifier
        try:
            # Use verify_identity method which returns proper dict
            result = self.verifier.verify_identity(image1, image2)
            
            # Handle case where no face detected
            if 'error' in result:
                return {
                    'match': False,
                    'similarity': 0.0,
                    'threshold': self.threshold,
                    'confidence_level': "No Face Detected",
                    'decision': "NO FACE DETECTED",
                    'recommendation': "REJECT - Face detection failed in one or both images",
                    'error': result['error']
                }
            
            similarity = result.get('match_score', 0.0)  # Changed from 'similarity' to 'match_score'
            match = result.get('is_same_person', False)  # Changed from 'match' to 'is_same_person'
            ...
            return {
                ...
                'decision': "ERROR",
                'recommendation': f"REJECT - Error during verification: {str(e)}"
            }
